...
def get_down_block(
    ...
    if down_block_type == "DownBlock2D":
        ...
        # <note> add extra inputs and outputs
...
class UNetMidBlock2DCrossAttn(nn.Module):
            ...
            if not dual_cross_attention:
                attentions.append(
                    Transformer2DModel(
                        ...
                        use_linear_projection=use_linear_projection,
                        upcast_attention=upcast_attention,
                        image_prompt_settings=image_prompt_settings, # <notice>
                    )
                )
            else:
                attentions.append(
    ...
    def forward(
        ...
        extra_dict_inputs = False,
        return_self_attn_map = False,
    ) -> torch.FloatTensor:
        extra_dict_outputs = {}
        hidden_states = self.resnets[0](hidden_states, temb)
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            hidden_states, extra_dict_output_attn = attn(
                hidden_states,
    ...
    def forward(
        ...
        for i, (resnet, attn) in enumerate(blocks):
            if self.training and self.gradient_checkpointing:
                ...
                # print("attn: ", attn)
                hidden_states, extra_dict_output_attn = torch.utils.checkpoint.checkpoint(
        ...
        return hidden_states, output_states, extra_dict_outputs
...
class DownBlock2D(nn.Module):
    def __init__(
        ...
        # self.extra_down_transformer_layer = image_prompt_settings.get("extra_down_transformer_layer", 0)
        # self.cross_attention_dim = image_prompt_settings.get("cross_attention_dim", 2048)
        # self.num_attention_heads = image_prompt_settings.get("num_attention_heads", 5)
        ...
        for i in range(num_layers):
            ...
            # if self.extra_down_transformer_layer > 0:
            #     attentions.append(
            #         Transformer2DModel(
            #             self.num_attention_heads,
            #             out_channels // self.num_attention_heads,
            #             in_channels=out_channels,
            #             num_layers=self.extra_down_transformer_layer,
    ...
    def forward(self, hidden_states, temb=None, extra_dict_inputs={}):
        extra_dict_outputs = {}  # <notice>
        output_states = ()
